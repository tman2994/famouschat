...
class QuickBloxHelper: NSObject, QBChatDelegate, QBRTCClientDelegate, QBRTCBaseClientDelegate, QBRTCAudioSessionDelegate
{
    ...
    func logoutUser() {
        QBRequest.logOut(successBlock: { (qbResponse) in
            print("------------ You are Logout from the Session")
        }) { (qbResponse) in
    ...
    func deleteCurrentUser() {
        ...
        logoutGroup.notify(queue: .main) {
            ...
            QBRequest.deleteCurrentUser(successBlock: { response in
                print("it is Sucessfully deleted from the Quicblox")
    ...
    func acceptCall()
    {
        ...
        if let _ = QuickBloxHelper.sharedInstance.newSession
        {
            ...
            UIApplication.shared.keyWindow?.setRootViewController(MainVideoChatVC(), options: UIWindow.TransitionOptions(direction: .toRight, style: .easeIn))
        }
    }
    ...
    func showCallingView(hintText: String, tutorId: String, videoCallId: String)
    {
        ...
        UIApplication.shared.keyWindow?.setRootViewController(MainVideoReceiveVC(), options: UIWindow.TransitionOptions(direction: .toRight, style: .easeIn))
    ...
    func session(_ session: QBRTCSession, rejectedByUser userID: NSNumber, userInfo: [String : String]? = nil)
    {
        if callDidEndCallBack != nil
        {
            ...
            CommonFuncs().doneAlert(ShareData.appTitle, "You can`t call \(ShareData.selected_book.profile.name!) now", "CLOSE", {UIApplication.shared.keyWindow?.setRootViewController(MainTabVC(), options: UIWindow.TransitionOptions(direction: .toLeft, style: .easeIn))})
