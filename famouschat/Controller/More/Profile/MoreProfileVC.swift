...
//  famouschat
//
//  Created by Oni Angel on 11/11/2018.
...
class MoreProfileVC: UIViewController, NVActivityIndicatorViewable {
    ...
    @IBOutlet weak var photo_edit_frame: UIView!
    @IBOutlet weak var photo_edit_btn: UIButton!
    @IBOutlet weak var profile_edit_frame: UIView!
    @IBOutlet weak var profile_edit_img: UIImageView!
    ...
    func init_UI()
    {
        ...
        photo_edit_frame.layer.cornerRadius = CGFloat(ShareData.btn_radius)
        photo_edit_frame.clipsToBounds = true
        
        let photoImg = UIImage.init(named: "camera1.png")?.withRenderingMode(.alwaysTemplate)
        photo_edit_btn.setImage(photoImg, for: .normal)
        photo_edit_btn.tintColor = UIColor.white
        
        profile_edit_frame.layer.cornerRadius = CGFloat(ShareData.btn_radius)
        profile_edit_frame.clipsToBounds = true
        
        let editImg = UIImage.init(named: "edit.png")?.withRenderingMode(.alwaysTemplate)
        profile_edit_img.image = editImg
        profile_edit_img.tintColor = UIColor.white
        
        profile_frame.dropShadow(color: .gray, opacity: 0.8, offSet: CGSize.zero, radius: 5, scale: true)
        
        name.text = ShareData.user_info.name!
        if ShareData.user_info.name!.contains(" ")
        {
            let array = ShareData.user_info.name!.characters.split{$0 == " "}.map(String.init)
            job.text = "@" + CommonFuncs().strArrayTostr(array, "")
        }
        else
        {
            job.text = "@" + ShareData.user_info.name!
        }
    ...
    @IBAction func profileEditAction(_ sender: Any) {
        ...
        UIApplication.shared.keyWindow?.setRootViewController(StartCategoryVC(), options: UIWindow.TransitionOptions(direction: .toRight, style: .easeIn))
    }
    ...
    @objc func backAction(_ sender: UIButton) {
        ...
        UIApplication.shared.keyWindow?.setRootViewController(MainTabVC(), options: UIWindow.TransitionOptions(direction: .toLeft, style: .easeIn))
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if tab_index == 0
        {
            ...
            if data.type! == ShareData.feed_type().promotion
            {
                ...
                return CommonFuncs().getStringHeight(CommonFuncs().splitString(data.optional_value!)[2], 15, UIScreen.main.bounds.width, 100, 220)
            }
            
            if array[1] == "1"
            {
                return CommonFuncs().getStringHeight(array[0], 15, UIScreen.main.bounds.width, 100, 30)
            }
            else
            {
                return CommonFuncs().getStringHeight(array[0], 13, UIScreen.main.bounds.width, 110, 65)
            }
        }
        else
        {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        if tab_index == 0
        {
            ...
            if data.type! == ShareData.feed_type().promotion
            {
                ...
                cell.photo.layer.borderColor = UIColor.init(red: 85/255, green: 155/255, blue: 255/255, alpha: 0.3).cgColor
            ...
            if array[1] == "1"
            {
                ...
                cell.warn_view.layer.borderWidth = 3
                cell.warn_view.layer.borderColor = UIColor.white.withAlphaComponent(0.5).cgColor
