...
class MainTabVC: UIViewController {
    ...
    func init_UI()
    {
        ...
        layerGradient.colors = [Utility.color(withHexString: "255681")!.cgColor, Utility.color(withHexString: "5688b3")!.cgColor]
        layerGradient.locations = [0.0, 1.0]
        layerGradient.frame = CGRect(x: 0, y: 0, width: view.bounds.width, height: view.bounds.height)
        tabBarView.tabBar.layer.addSublayer(layerGradient)
        
        tabBarView.tabBar.unselectedItemTintColor = UIColor.white
        tabBarView.tabBar.selectedImageTintColor = Utility.color(withHexString: ShareData.btn_color)
