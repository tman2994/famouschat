...
class MainHomeVC: UIViewController, NVActivityIndicatorViewable, UIWebViewDelegate {
    ...
    func init_UI()
    {
        ...
        if time > 5 && time < 12
        {
            hello_label.text = "Goood Morning"
        }
        else if time >= 12 && time < 18
        {
            hello_label.text = "Good Afternoon"
        }
        else if time >= 18 && time < 23
        {
            hello_label.text = "Good Evening"
        }
        else
        {
            hello_label.text = "Good Night"
        }
...
extension MainHomeVC: UITableViewDelegate, UITableViewDataSource
{
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if array[1] == "1"
        {
            return CommonFuncs().getStringHeight(array[0], 15, UIScreen.main.bounds.width, 100, 50)
        }
        else
        {
...
//        video_view.isLoop = true
    ...
    @objc func reviewView(_ sender: UIButton) {
        ...
        UIApplication.shared.keyWindow?.setRootViewController(MoreRateReviewVC(), options: UIWindow.TransitionOptions(direction: .toRight, style: .easeIn))
    }
    ...
    @objc func coinBuy(_ sender: UIButton) {
        ...
        self.feedRemove(index: self.selected_index)
        ShareData.pay_from_status = true
        UIApplication.shared.keyWindow?.setRootViewController(MorePayVC(), options: UIWindow.TransitionOptions(direction: .toRight, style: .easeIn))
    }
