...
class MainVideoChatVC: UIViewController {
    ...
    @IBAction func timeAddAction(_ sender: Any) {
        ...
        let array3 = CommonFuncs().splitString(ShareData.selected_book.profile.chat_rate!)
        let fee_rate = (Double(array3[2])! + Double(array3[3])! / 100) / (Double(array3[0])! * 60 + Double(array3[1])!)
        
        more_time_money.text = "$\(String(format: "%.02f", fee_rate * Double(more_time)))"
        
    }
    
    @IBAction func moreBuyAction(_ sender: Any) {
        
        let array = CommonFuncs().splitString(ShareData.selected_book.profile.chat_rate!)
        let fee_rate = (Double(array[2])! + Double(array[3])! / 100) / (Double(array[0])! * 60 + Double(array[1])!)
        
        let coin_num = Int(fee_rate * Double(more_time) / Double(ShareData.app_config.popcoin_price!)!)
