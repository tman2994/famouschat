                    ...
                    uploadRequest.responseJSON(completionHandler: {response in
                        ...
                        if let JSON = response.result.value
                        {
                            ...
                            if status
                            {
                                ...
                                CommonFuncs().doneAlert(ShareData.appTitle, "You have successfully updated user profile", "DONE", {UIApplication.shared.keyWindow?.setRootViewController(MainTabVC(), options: UIWindow.TransitionOptions(direction: .toRight, style: .easeIn))})
                                
                            }
                            else
                            {
                                self.stopAnimating(nil)
                                CommonFuncs().doneAlert(ShareData.appTitle, message, "CLOSE", {UIApplication.shared.keyWindow?.setRootViewController(MainTabVC(), options: UIWindow.TransitionOptions(direction: .toRight, style: .easeIn))})
