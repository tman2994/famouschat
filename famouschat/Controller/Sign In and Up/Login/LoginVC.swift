...
class LoginVC: UIViewController, NVActivityIndicatorViewable{
    ...
    @IBAction func helpLoginAction(_ sender: Any) {
        ...
        self.forget_email_in = CommonFuncs().inputAlert(ShareData.appTitle, "Please input email of your account", "Email Address...", 2, ["OK", "CANCEL"], [Utility.color(withHexString: ShareData.btn_color), UIColor.gray], [UIColor.white, UIColor.white], self, [#selector(self.forgetOk(_:)), #selector(self.forgetCancel(_:))])
        forget_status = "1"
...
extension LoginVC
{
        ...
        CommonFuncs().createRequest(false, "login/forget", "POST", parmeters, completionHandler: {data in
            ...
            DispatchQueue.main.async {
                ...
                if status
                {
                    ...
                    self.stopAnimating(nil)
                    UIApplication.shared.keyWindow?.setRootViewController(MainTabVC(), options: UIWindow.TransitionOptions(direction: .toRight, style: .easeIn))
                }
                else
                {
                    self.stopAnimating(nil)
    ...
    func googoleSign(user: GIDGoogleUser!){
